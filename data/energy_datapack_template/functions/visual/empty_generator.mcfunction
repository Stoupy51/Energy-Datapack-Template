...
#Update model depends on BurnTime (on/off)
	execute if score #success energy_datapack_template.data matches 1 run data modify entity @s Item.tag.CustomModelData set value XXX0300
	execute if score #success energy_datapack_template.data matches 0 run data modify entity @s Item.tag.CustomModelData set value XXX0301
